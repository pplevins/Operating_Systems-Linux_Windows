...
sem_t sema; /*declaring the semaphore. the initialization is in main.*/
/**********************************************************************/
...
int main(int ac,char **av ) 
{
  ...
  num_agents = init( i );
  sem_init(&sema, 0, 1); /*intializing the semaphore*/
  ...
  while ( p >= 1 ) {
    sleep(rand()%4+1);
    check_point( &i );} //!!!Have to transfer to check_point, and now it's randomly.
...
int update(int amount,char *account, char * name )  
{
   ...
   sem_wait(&sema); //entering the critical section.
   ...
   fclose( fp );
   sem_post(&sema); //exiting the critical section.
